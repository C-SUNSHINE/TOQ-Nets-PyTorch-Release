...
from toqnets.utils import plot_dependency


class NLTL_GNN_SAv3(nn.Module):
    default_config = {
        'name': 'NLTL_GNN_SAv3',
        'state_dim': 3,
        'type_dim': 4,
        'image_dim': None,
        'n_actions': 9,
        'input_transformer': 'stgcn',
        'depth': 3,
        't_depth': 3,
        'default_temporal_integrator': 'middle',
    ...
    def __init__(self, config):
        ...
        if input_transformer == 'stgcn':
            h_dim = 256
            dropout = 0.5
            kernel_size = (1, 9)
            self.stgcn_layers = nn.ModuleList([
                STGCN_Layer(state_dim, h_dim, kernel_size, 1, residual=False),
                STGCN_Layer(h_dim, h_dim, kernel_size, 1, dropout=dropout, residual=True),
                STGCN_Layer(h_dim, h_dim, kernel_size, 1, dropout=dropout, residual=True),
                STGCN_Layer(h_dim, h_dim, kernel_size, 1, dropout=dropout, residual=True),
    ...
    def forward(self, data=None, hp=None):
        ...
        if self.config['input_transformer'] == 'stgcn':
            ...
            inputs_to_transform = nn.Sigmoid()(inputs_to_transform)
        ...
        if self.config['depth'] > 0:
            ...
            nltl_input = self.logic_machine0(nlm_inputs[1].mean(dim=2))
