...
class NONLOCAL_SA(nn.Module):
    ...
    def __init__(self, config):
        ...
        if small_model:
            ...
            h_dim = self.config['h_dim'] = 96
        if tiny_model:
            n_features = self.config['n_features'] = 64
            h_dim = self.config['h_dim'] = 48
        ...
        self.encoder = nn.Sequential(nn.Linear(state_dim + add_state_dim + type_dim + tim_dim, h_dim), nn.ReLU(),
                                     nn.Linear(h_dim, h_dim))
        self.non_local = NonLocal(h_dim, n_features, t_kernel_size=t_kernel_size, small_model=small_model,
    ...
    def reset_parameters(self):
        init_weights(self.non_local.non_local_layers[0])
